...
class ReadMangaToday extends AbstractCatalog {
    ...
    popularMangaList ($: CheerioObject): Array<Manga> {
        ...
        $('div.hot-manga > div.style-list > div.box').each((i: number, elem: CheerioObject) => {
            let manga: Manga = this.extractMangaSummary($, elem, provider.getNextIndex());
    ...
    popularMangaPaginator ($: CheerioObject): {hasNext: boolean, nextUrl: string} {
        let pagination: CheerioObject = $('div.hot-manga > ul.pagination > li > a:contains(»)');
    ...
    mangaDetail ($: CheerioObject, manga: Manga): Manga {
        ...
        manga.genre = trimSpaces(container.find('dl.dl-horizontal > dd').eq(2).text());
        manga.description = trimSpaces(container.find('li.movie-detail').text());
        manga.status = trimSpaces(container.find('dl.dl-horizontal > dd').eq(1).text());
        manga.thumbnailUrl = trimSpaces(container.find('img.img-responsive').attr('src'));
    ...
    chapterList ($: CheerioObject, manga: Manga): Array<Chapter> {
        ...
        $('ul.chp_lst > li').each((i, elem) => {
            ...
            chapter.title = trimSpaces($(elem).find('a').first().find('span.val').text());
            chapter.publishedAt = parseDateAgo(trimSpaces($(elem).find('span.dte').first().text()));
     ...
     * @returns {Manga}
     */
    extractMangaSummary ($: CheerioObject, elem: CheerioObject, catalogId: ?number): Manga {
